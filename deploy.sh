...
helm list | grep -q bootstrap-app &> /dev/null || echo 'waiting 45 seconds for argo to deploy' && sleep 45 && helm upgrade -n argocd --install bootstrap-app apps/ -f ./apps/values.yaml
